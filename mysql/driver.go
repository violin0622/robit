...
func (d *Driver[ID]) Acquire(
	...
	results, err := d.electStmt.ExecContext(ctx, d.subject, id, ttl.Milliseconds())
	if err != nil {
		return nil, fmt.Errorf(`Acquire: %w`, err)
	}
	...
	case err != nil:
		return nil, fmt.Errorf(`Acquire: %w`, err)
	case ra == 1, ra == 2:
		// ra=1: new row inserted; ra=2: took over an expired lock.
		return d.spawnLease(ctx, id, ttl)
	case ra == 0:
		// Lock is actively held; determine who holds it.
		return nil, d.checkHolder(ctx, id)
	default: // unreachable
		return nil, nil
	}
}
...
// checkHolder queries the current lock holder when Acquire gets ra=0.
// This is a best-effort check: the lock is confirmed held (by the atomic
// INSERT), we only need to determine the error type for the caller.
func (d *Driver[ID]) checkHolder(ctx context.Context, id ID) error {
	var holder ID
	err := d.db.QueryRowContext(ctx, d.checkHolderQuery, id).Scan(&holder)
	switch {
	case err == sql.ErrNoRows:
		// Lock expired between the INSERT and this SELECT; report as acquired-by-other.
		return robit.ErrAcquireFailed
	case err != nil:
		return fmt.Errorf(`check holder: %w`, err)
	case holder == d.subject:
		return robit.ErrAlreadyHeld
	default:
		return robit.ErrAcquireFailed
	}
}
...
// e.g. "table_name", "subject_id", "object_id", "expires_at",
// "表名", "主体", "客体", "过期时刻" ...
// but not "table name", "subject-id", "object.id", "expires`at"
...
// RowsAffected: 1=inserted, 2=took over expired, 0=lock actively held.
const electTpl = "" +
	"INSERT INTO `{{.Table}}` (`{{.Subject}}`, `{{.Object}}`, `{{.ExpiresAt}}`) " +
	"VALUES (?, ?, FLOOR(UNIX_TIMESTAMP(NOW(3)) * 1000) + ?) " +
	"ON DUPLICATE KEY UPDATE " +
	"`{{.Subject}}` = IF(`{{.ExpiresAt}}` < FLOOR(UNIX_TIMESTAMP(NOW(3)) * 1000), VALUES(`{{.Subject}}`), `{{.Subject}}`), " +
	"`{{.ExpiresAt}}` = IF(`{{.ExpiresAt}}` < FLOOR(UNIX_TIMESTAMP(NOW(3)) * 1000), VALUES(`{{.ExpiresAt}}`), `{{.ExpiresAt}}`);"
...
const refreshTpl = "" +
	"UPDATE `{{.Table}}` " +
	"SET `{{.ExpiresAt}}` = FLOOR(UNIX_TIMESTAMP(NOW(3)) * 1000) + ? " +
	"WHERE `{{.Subject}}` = ? " +
	"AND `{{.Object}}` = ?;"
...
const checkTpl = "" +
	...
	"WHERE `{{.Subject}}` = ? " +
	"AND `{{.Object}}` = ? " +
	"AND `{{.ExpiresAt}}` >= FLOOR(UNIX_TIMESTAMP(NOW(3)) * 1000);"
...
// Params: object(?1).
// Used on Acquire failure (ra=0) to distinguish ErrAlreadyHeld vs ErrAcquireFailed.
...
// releaseTpl deletes the lock row. The subject condition prevents releasing
// a lock that has been taken over by another holder.
// Params: subject(?1), object(?2).
const releaseTpl = "" +
	"DELETE FROM `{{.Table}}` " +
	"WHERE `{{.Subject}}` = ? " +
